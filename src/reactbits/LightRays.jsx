...
uniform vec2 rayPos; uniform vec2 rayDir;
uniform vec3 raysColor; uniform float raysSpeed;
uniform float lightSpread; uniform float rayLength;
uniform float pulsating; uniform float fadeDistance;
uniform float saturation; uniform vec2 mousePos;
uniform float mouseInfluence; uniform float noiseAmount;
...
void main(){ vec4 color; mainImage(color, gl_FragCoord.xy); gl_FragColor = color; }`;
        ...
        catch (e) { console.warn('WebGL rendering error:', e); }
        };
        ...
        try {
            ...
            gl2.canvas?.parentNode?.removeChild(gl2.canvas);
        } catch {}
    ...
    const u = uniformsRef.current, renderer = rendererRef.current;
    ...
    return (
    <div ref={containerRef} className={`w-full h-full pointer-events-none z-[3] overflow-hidden relative ${className}`.trim()} />
    );
}