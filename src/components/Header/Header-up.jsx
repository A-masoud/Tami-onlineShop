...
export default function HeaderUp() {
  ...
  // درخواست پیشنهادها (مثال با fetch ساده، بک‌اند باید آماده باشه)
  useEffect(() => {
    const fetchSuggestions = async () => {
      ...
      try {
        ...
        setSuggestions(data.slice(0, 5)); // فقط 5 پیشنهاد اول
      } catch (err) {
        console.error(err);
      }
    };

    const timeout = setTimeout(fetchSuggestions, 300); // debounce ساده
        ...
        <div className="mx-auto max-w-none px-6">
          <div className="h-16 flex items-center justify-between gap-4">
                ...
                <div className="absolute right-0 top-full min-w-56 pt-2 z-50">
                  ...
                  >
                    {['بهداشت','آرایشی','خانه و سبک زندگی','سوپرمارکت'].map(item => (
                      <a key={item} href="#" className="block px-3 py-2 rounded-xl text-white/80 hover:text-white hover:bg-white/10">{item}</a>
                    ))}
                  </div>
                </div>
              </div>  
            ...
            <div className="flex items-center gap-2 relative">
              ...
              <a href="/login" className="hidden md:inline-flex items-center gap-2 rounded-2xl border border-white/15 bg-white/5 hover:bg-white/10 text-white px-3 py-2 transition" title="ورود / ثبت‌نام">
